...
interface PropiedadesDetalleServicio {
    ...
    <Box component="section" sx={{ py: { xs: 6.5, md: 10 }, color: "common.white", background: "linear-gradient(135deg, #55275D, #8D3D9A)" }}>
      ...
      <PageContainer sx={{ display: "grid", gridTemplateColumns: { xs: "1fr", md: "1fr 1fr" }, alignItems: "center", gap: { xs: 6, md: 8 } }}>
        <Stack alignItems="flex-start">
          ...
          <Typography variant="h1" sx={{ mb: 2, fontSize: { xs: "3.1rem", md: "5rem" }, lineHeight: 1 }}>{servicio.titulo}</Typography>
          {servicio.rangoEdad && <Chip label={servicio.rangoEdad} sx={{ mb: 2.5, bgcolor: "rgba(255,255,255,.13)", color: "common.white", fontWeight: 800 }} />}
          <Typography sx={{ maxWidth: 600, mb: 4, color: "rgba(255,255,255,.8)", fontSize: "1.04rem", lineHeight: 1.8 }}>{servicio.introduccion}</Typography>
          <ButtonLink destino={urlReservaWhatsapp} externo variante="claro" endIcon={<ArrowForwardRoundedIcon />}>Consultar por este servicio</ButtonLink>
        </Stack>

        <Box sx={{ position: "relative", pb: { xs: 4.5, sm: 0 } }}>
          <Box component="img" src={servicio.imagen} alt={`Servicio de ${servicio.titulo}`} sx={{ width: "100%", height: { xs: 340, sm: 440, md: 560 }, objectFit: "cover", borderRadius: 4, boxShadow: "0 22px 55px rgba(44,17,50,.28)" }} />
          <Box sx={{ position: "absolute", left: { xs: 1.75, sm: "auto" }, right: { xs: 1.75, sm: 2.25 }, bottom: { xs: 0, sm: 2.25 }, maxWidth: { sm: 280 }, p: 2.5, borderRadius: 2.5, bgcolor: "common.white", color: "primary.dark", boxShadow: "0 18px 40px rgba(44,17,50,.22)" }}>
            ...
            <Typography fontWeight={800}>{servicio.prestaciones[0]?.titulo ?? "Consulta personalizada"}</Typography>
          </Box>
        </Box>
      </PageContainer>
    </Box>
